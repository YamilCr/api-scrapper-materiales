...
async def fetch_data_layer_items(search: str, limit: int = 20):
    ...
    url = f"https://www.masonline.com.ar/fernet%20branca?_q=fernet+branca"
    ...
    async with async_playwright() as p:
        try:
            ...
            await page.wait_for_selector('.valtech-gdn-dynamic-product-1-x-weighablePriceWrapper', timeout=10000)
    ...
    product_divs = soup.find("div", class_="valtech-gdn-search-result-0-x-gallery")
    # print(product_divs)
    ...
    products_div = product_divs.find_all(
        "div",
        class_="valtech-gdn-search-result-0-x-galleryItem"
    )
    
    products = []
    end_time = time.time()  # <--- fin del timer
    print(f"[Servidor - consulta a mas online] Tiempo de respuesta de la peticion: {end_time - start_time:.2f} segundos")  # <--- solo para logs
    # limite de respuesta
...
def extract_product_data(product_div):
    ...
    try:
        # Nombre del producto
        name_tag = product_div.find("span", class_="vtex-product-summary-2-x-productBrand")
        name = name_tag.text.strip() if name_tag else None

        # Precio reconstruido desde múltiples spans (VTEX - MásOnline)
        price_container = product_div.find("div", class_="valtech-gdn-dynamic-product-1-x-dynamicProductPrice")
        
        price_number = None

        if price_container:
            currency_integers = price_container.find_all("span", class_="valtech-gdn-dynamic-product-1-x-currencyInteger")
            currency_group = price_container.find("span", class_="valtech-gdn-dynamic-product-1-x-currencyGroup")
            currency_fraction = price_container.find("span", class_="valtech-gdn-dynamic-product-1-x-currencyFraction")

            if currency_integers and currency_group and currency_fraction:
                # Ejemplo: 4.599,00 → integer_part = "4599", decimal_part = "00"
                integer_part = "".join([span.text.strip() for span in currency_integers])
        ...
        image_url = image_tag.get("src") if image_tag and image_tag.get("src") else "https://carrefour.com.ar/default-image.svg"

        # Link al producto
        link_tag = product_div.find("a", href=True)
        link = f"https://www.masonline.com.ar{link_tag['href']}" if link_tag else None

        # Marca (si está disponible)
        brand_tag = product_div.find("span", class_="vtex-product-summary-2-x-brandName")
...
# products = asyncio.run(fetch_data_layer_items("arroz luchetti"))
